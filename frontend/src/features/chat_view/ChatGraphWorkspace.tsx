...
import type { ChatCheckpointState, ChatTurnGraph, ChatTurnStateHistory } from "../chat/types";

type DebugTab = "graph" | "checkpoints" | "context" | "performance";

type DiffKind = "added" | "removed" | "changed" | "same";

interface ChatGraphWorkspaceProps {
  ...
  onClose: () => void;
  stateHistory: ChatTurnStateHistory | null;
}

export function ChatGraphWorkspace({
  graph,
  historyError = "",
  isClosing = false,
  ...
  useEffect(() => {
    ...
    const ordered = [...stateHistory.states].reverse();
    const latest = ordered[ordered.length - 1];
    setHeadId(latest?.checkpoint_id ?? null);
  }, [stateHistory]);
