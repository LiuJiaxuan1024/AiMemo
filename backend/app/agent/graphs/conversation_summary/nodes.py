        ...
        [
            SystemMessage(
                content=(
                    ...
                    "必须遵守：1. 不要复制原始对话全文，不要输出“旧摘要/新增消息”等标签；"
                    "2. 不要把旧任务写成当前任务，当前任务只能来自新增消息末尾仍在继续的主题；"
                    "3. 如果新增消息已经切换主题，旧主题必须降级为历史背景或已解决事项；"
                    "4. 保留用户明确偏好、仍未解决的问题、最近正在处理的目标和重要事实；"
                    "5. 忽略寒暄、重复表达和无意义细节，不要编造。"
                    "请使用中文，控制在 1200 字以内。"
                )
            ),
            HumanMessage(
