...
这种结构适合开发者，但对普通用户不友好。很多配置本质上是产品内的偏好或开关，例如“把聊天模型换成某个模型”“关闭精灵”“这轮对话挂载某个知识空间”“把精灵声音改成某个声线”。这些操作应该可以通过对话完成。
...
6. 指令配置结果必须能触发对应运行时 reload，例如模型 cache reset、精灵状态刷新。
...
显式斜杠指令必须由轻量 parser / command router 确定性解析。parser 的职责是把 `/config elf.enabled false` 这类稳定格式直接转成配置工具调用参数，绕过 LLM 意图分析。
...
- 必填参数缺失，例如 `/config elf.enabled` 没有给 `true|false`。
...
```json
{
  ...
  "questions": [
    {
      "id": "elf_enabled",
      "question": "精灵开关要设置为哪个值？",
      "selection_mode": "single",
      "options": [
        {"id": "true", "label": "开启", "value": "true"},
        {"id": "false", "label": "关闭", "value": "false"}
      ]
    }
  ]
}
...
- `set_elf_enabled` / `set_default_voice_profile` / `set_elf_voice_mode`：精灵专用配置。
...
```json
{
  ...
  "type": "command_result",
  "command": "/config elf.enabled false",
  "status": "success",
  "scope": "user",
  "changed": true,
  "target": "elf.enabled",
  ...
  "message": "精灵已关闭，刷新后不会加载。",
  "audit_id": "audit_xxx",
  "rollback_command": "/config elf.enabled true"
}
```
...
Layer 2: runtime config
  数据库或受控 JSON5 写入，用于用户偏好、模型 slot、精灵开关。
...
其中 `elf.enabled` 必须影响前端和桌面：

- 前端打开后是否挂载 `ElfAssistant`。
- 桌面 wrapper 是否创建外置精灵窗口。
- Runtime Config API 是否返回一致状态。
...
```text
已关闭精灵。可用 /config elf.enabled true 重新开启。
```
...
- 每个候选展示名称、描述、作用域和风险。
- 支持 fuzzy search，例如输入 `/elf` 能搜到 `/config elf.enabled`。
...
- 精灵关闭时，精灵相关只读状态仍可显示，依赖精灵运行态的动作指令应禁用。
...
需要联动 Runtime Config API 和桌面 wrapper。

Step 3 的目标不是只在 command router 里硬编码三个 executor，而是把精灵相关配置收束到一套受控配置服务中，
让 `/` 指令、自然语言工具调用、设置页和桌面 wrapper 都读写同一份状态。
...
```text
elf.enabled
  是否加载精灵本体。影响前端是否挂载 ElfAssistant，桌面端是否创建精灵窗口。
  作用域：user。风险：medium。可回滚。
...
- `config.json5`：`elf.enabled` 的启动配置来源。
...
这样 `elf.enabled` 可以先保留 `config.json5` 作为默认值，但用户通过指令修改后写入数据库 runtime config；Runtime Config API、command router、前端和桌面 wrapper 都读取 effective value。`elf.voice.mode` 也应从内存态迁移到 runtime config，避免刷新或重启后状态丢失。

#### Step 3.3：命令行为

`/config elf.enabled <true|false>`

- 参数缺失：返回 `needs_input`，用单选卡展示“开启 / 关闭”。
- 参数非法：返回 `failed`，只提示 `elf.enabled 只接受 true 或 false`，不展示候选项。
- 设置为当前值：返回 `noop`。
- 设置成功：写入 runtime config，返回 `success`，`rollback_command` 为相反值。
- reload：触发 `runtime_config` 和 `elf_state`。前端应收到新 runtime config 后挂载或卸载精灵；桌面 wrapper 下一次状态轮询或配置刷新后同步。
...
1. 增加 runtime config service 和最小数据库表，先支持 `elf.enabled` / `elf.voice.mode` 两个布尔值。
2. 改 Runtime Config API，让前端和桌面都读 effective runtime config。
3. 在 command registry 开启 `config.elf.enabled`，新增 `config.elf.voice.mode` 和 `config.elf.voice.default`。
4. 在 command router 接入三个 executor，并复用通用布尔解析、声线解析和 `needs_input` 结果构造。
