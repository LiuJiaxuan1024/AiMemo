...
## 可借鉴 Claude Code 的设计点

本项目不照搬 Claude Code 的实现，但可以吸收它的几个工程原则。

### 1. 工具是模型可见的动作，不是隐藏上下文

Claude Code 的核心模式是：模型显式选择工具，工具结果以 `tool_result` 形式回到对话，再让模型继续推理。这样模型不会只看到“整理后的摘要”，而是能根据真实结果继续行动。
...
Claude Code 的 Read 工具心智是：如果用户给了路径，模型应该尝试读取；路径不存在、权限不足、不是文本等问题由工具返回错误。
...
Claude Code 的 Write 工具要求：如果目标文件已存在，必须先读该文件，否则写入失败。这个规则非常适合 Ai 记。
...
Claude Code 的权限判断不是写在 prompt 里让模型自觉遵守，而是在工具执行前做独立检查。
...
### 与 Claude Code 展示方式的关系

Claude Code 的交互给我们的启发是：用户不只关心最后答案，也关心 agent 正在执行哪些步骤。Ai 记可以在此基础上做得更柔和：
