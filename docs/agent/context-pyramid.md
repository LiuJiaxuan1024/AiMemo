...
backend/app/agent/context/pyramid.py
  ...
  负责把 L0-L4 五层上下文组装成 prompt_context。

backend/app/agent/graphs/memory_chat/nodes.py
  dispatch_context_workers 使用 LangGraph Send 分发 L0-L4 worker。
...
flowchart TB
    ...
    L1[L1 近期对话窗口<br/>最近消息，按 token 预算裁剪] --> Prompt
...
五层之间数据依赖很少，天然适合并行。每个 worker 写入独立 state 字段：
...
`merge_prompt_context` 按 L4 -> L0 顺序还原为 `PyramidPromptContext`，并渲染为最终 `prompt_context`。
