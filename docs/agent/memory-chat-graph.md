...
flowchart TD
    ...
    Dispatch --> K35[build_l3_knowledge_context]
...
build_l3_retrieved_memory
  L3 内部默认每轮执行 cheap_note_recall -> retrieval_gate -> grade_retrieval -> build layer。
  cheap recall 不请求 embedding，用关键词、标题、标签、摘要和 chunk 文本做低延迟召回。
  明确个人记忆意图、个人画像问题，或显式注入 planner 要求时，才升级到向量检索 retrieve_notes。
  这是个人笔记检索层，避免“看似常识题、实际关联用户笔记”的问题丢失个人语境，同时避免每轮 5s 级向量检索阻塞。

build_l3_knowledge_context
  读取当前 conversation 已挂载的知识空间，构建 L3.5 挂载知识空间检索上下文。
...
个人笔记检索从“每轮强制向量检索”改为“每轮 cheap recall + 保守门控 + 必要时向量检索”。这样即使用户问的是一个看似简单的科学性或常识问题，系统也能先用低成本关键词召回个人笔记里的 top-k 候选，再由 agent 判断这些候选是否真的有用。

```text
默认生产路径
  -> 使用用户原文作为 retrieval_query
  -> search_notes_keyword(top_k=5)
  -> retrieval_gate(skip / light / vector)
  -> grade_retrieval
  -> build L3 个人笔记检索层

升级向量检索路径
  -> 明确个人记忆查询 / 个人画像 / 显式注入 planner 要求
  -> retrieve_notes(top_k=5)
  -> grade_retrieval
  -> build L3 个人笔记检索层
```

检索门控不是问“需不需要检索”，而是问“能不能安全跳过重检索”。当前规则：
...
light
  默认路径。使用 cheap recall 的候选，不请求 embedding。

vector
  用户明确提到我之前、上次、笔记、记录、来着、个人画像等个人记忆意图；
  或显式注入 planner 要求向量检索。
```
...
生产默认路径已经跳过 L3 内部的 planner LLM，也不会每轮请求 embedding。只有 action=vector 时才会进入向量检索。
...
这个日志用于继续判断 L3 的实际慢点：生产默认路径中 `planner_ms` 应接近 0；
如果 `cheap_recall_ms` 高，说明关键词召回需要优化；如果 `retriever_ms` 高，说明 embedding 或 sqlite-vec 查询慢。
...
当用户使用“那个”“来着”等指代或省略表达时，显式注入的 planner 可以把问题改写为更适合检索的 `retrieval_query`，也可以显式升级向量检索；它不再是是否执行 cheap recall 的开关。
...
flowchart TD
    ...
    Dispatch --> K35[L3.5 worker<br/>mounted knowledge search]
    Dispatch --> L3[L3 worker<br/>cheap recall / gate / vector / grade / build]
...
```mermaid
flowchart TD
    L3Start([L3 START]) --> Query[build retrieval_query]
    Query --> Cheap[search_notes_keyword]
    Cheap --> Gate{retrieval_gate}
    Gate -->|skip| Build
    Gate -->|light| Grade
    Gate -->|vector| Embed[query embedding]
