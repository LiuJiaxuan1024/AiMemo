...
build_l3_retrieved_memory
  L3 内部默认每轮执行 retrieve_notes -> grade_retrieval -> build layer。
  这是个人笔记检索层，避免“看似常识题、实际关联用户笔记”的问题丢失个人语境。
  默认不调用检索规划 LLM；planner 只在显式注入时作为 query rewrite 辅助，不能决定是否跳过检索。
...
个人笔记检索已经从“按需判断”改为“非空用户输入默认检索”。这样即使用户问的是一个看似简单的科学性或常识问题，系统也会先召回个人笔记里的 top-k 候选，再由 agent 判断这些候选是否真的有用。
...
显式注入 planner 的路径（测试或后续优化）
  -> planner 只负责 query rewrite
  -> 即使 planner 判断为 direct，也不能跳过个人笔记检索
```

保留的旧 planner 规则快路径仅用于 query rewrite 兼容：
...
生产默认路径已经跳过 L3 内部的 planner LLM，只保留 embedding 检索和最终回答生成。
...
当用户使用“那个”“来着”等指代或省略表达时，显式注入的 planner 可以把问题改写为更适合向量检索的 `retrieval_query`，但它不再是是否检索的开关。
...
flowchart TD
    ...
    Dispatch --> L3[L3 worker<br/>personal notes retrieve / grade / build]
