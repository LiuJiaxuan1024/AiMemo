...
本文记录 Ai 记精灵助手的产品定位、当前实现和历史方案。精灵不是单纯装饰物，而是 Ai 记后台 agent 系统的可视化媒介。

## 当前状态

当前推荐形态是桌面外置精灵，Web 内精灵作为历史兼容能力保留。精灵使用本地 PNG 表情资源，而不是直接加载远程 Live2D 模型。
...
早期 Web 内精灵方案建议替换右下角的精灵工坊入口表现，但不删除精灵工坊。当前主方向已经迁移到桌面外置精灵。
...
```text
继续以桌面外置精灵作为主入口。
Web 端保留 AiMemo 页面，不再强行内置精灵。
后端事件中心作为浏览器和桌面精灵的共同事件来源。
如果后续接 Live2D，只替换渲染层，不改事件和状态模型。
```

## 已完成记录

### 2026-05-17：第一版 Web 内精灵助手

以下记录是历史实现过程，部分内容已经被桌面外置精灵方案替代。
...
以下记录是历史修复过程。当前默认方案不再依赖远程 Live2D 示例模型。
